...
*/
extern const uint8_t adf_music_mp3_start[] asm("_binary_dingdong_mp3_start");
extern const uint8_t adf_music_mp3_end[]   asm("_binary_dingdong_mp3_end");
void stop_play_mp3(void);
void release_play_mp3(void);
static audio_pipeline_handle_t pipeline_play;
...
void init_play_mp3(void)
{
    ...
    audio_element_set_read_cb(mp3_decoder_play, mp3_music_read_cb, NULL);
...
#else
    audio_pipeline_link(pipeline_play, (const char *[]) {"mp3", "i2s"}, 2);
...
void start_play_mp3(void)
{
    ...
    while (1) {
        ...
        if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT && msg.source == (void *) mp3_decoder_play
            && msg.cmd == AEL_MSG_CMD_REPORT_MUSIC_INFO) {
            audio_element_info_t music_info = {0};
            audio_element_getinfo(mp3_decoder_play, &music_info);

            ESP_LOGI(TAG, "[ * ] Receive music info from mp3 decoder, sample_rates=%d, bits=%d, ch=%d",
                     music_info.sample_rates, music_info.bits, music_info.channels);

            audio_element_setinfo(i2s_stream_writer_play, &music_info);
...
#if (CONFIG_ESP_LYRATD_MSC_V2_1_BOARD || CONFIG_ESP_LYRATD_MSC_V2_2_BOARD)
#else
            i2s_stream_set_clk(i2s_stream_writer_play, music_info.sample_rates , music_info.bits, music_info.channels);
#endif
            continue;
        }
        /* Stop when the last pipeline element (i2s_stream_writer in this case) receives stop event */
        if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT && msg.source == (void *) i2s_stream_writer_play
